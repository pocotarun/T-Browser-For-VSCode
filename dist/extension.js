...
exports.TBrowserWorkspaceExtension = void 0;
...
class EnterpriseCookieJar {
    ...
    serializeCookiesForRequest(hostname) {
        const domainJar = this.hostCookieMap.get(hostname);
...
class JavascriptRuntimeInjector {
    ...
    static constructInjectionPayload(activeProxyPort, currentActiveOrigin) {
        return `
...
class HTMLStreamAssemblyLayer {
    ...
    static executeAssemblyAndInjection(upstreamResponse, clientResponse, activeProxyPort, currentActiveOrigin) {
        let readableStream = upstreamResponse;
    ...
    static executeGlobalRegexReplacement(htmlPayload, port, origin) {
...
class EnterpriseVirtualizationEngine {
    ...
    routeIncomingTraffic(incomingClientReq, outgoingClientRes) {
        try {
            ...
            if (incomingClientReq.url && incomingClientReq.url.startsWith(PROXY_ROUTE_PREFIX)) {
                ...
                try {
                    this.trackedActiveOrigin = new url_1.URL(finalTargetUrlString).origin;
                }
                ...
                if (requestReferer && requestReferer.includes(PROXY_ROUTE_PREFIX)) {
                    ...
                    if (splitRefererArray.length > 1) {
                        ...
                        try {
                            const refererOriginObject = new url_1.URL(decodedOriginalRefererUrl);
            ...
            let validatedTargetUrlObject;
            try {
                validatedTargetUrlObject = new url_1.URL(finalTargetUrlString);
            }
    ...
    dispatchUpstreamNetworkRequest(targetUrlObject, incomingClientReq, outgoingClientRes) {
        ...
        const activeHeaders = upstreamRequestOptions.headers;
    ...
    handleUpstreamNetworkResponse(targetUrlObject, upstreamResponse, outgoingClientRes) {
        // Strict cast to maintain index signatures correctly
        const cleanResponseHeaders = { ...upstreamResponse.headers };
        ...
        if (cleanResponseHeaders['set-cookie']) {
            ...
            cleanResponseHeaders['set-cookie'] = arrayCookies.map((cookieString) => {
        ...
        if (responseIsHtmlDocument) {
            ...
            HTMLStreamAssemblyLayer.executeAssemblyAndInjection(upstreamResponse, outgoingClientRes, this.assignedPort, this.trackedActiveOrigin);
        }
        else {
            outgoingClientRes.writeHead(upstreamResponse.statusCode || 200, cleanResponseHeaders);
    ...
    renderFallbackErrorView(outgoingClientRes, errorReason) {
        if (outgoingClientRes.headersSent) {
            return;
        }
...
class TBrowserWorkspaceExtension {
    static activeProxyEngine = null;
    // IMPLEMENTATION TRACKING: [VSCODE_EXTENSION] - [ACTIVATION_ROUTINE]
    static async triggerActivation(vscodeContext) {
        try {
    ...
    static instantiateWebviewPanel(vscodeContext, enginePort) {
        const vscodeWebviewPanel = vscode.window.createWebviewPanel('tBrowserEnterpriseEdition', 'T Browser', vscode.ViewColumn.Two, {
            enableScripts: true,
...
exports.TBrowserWorkspaceExtension = TBrowserWorkspaceExtension;
